...
class PokedexSilentRefreshService {
  ...
  static const int    _checkIntervalDays = 7;  // verifica uma vez por semana
  static const int    _sampleSize        = 20; // pokémon verificados por sessão
  ...
  /// Retorna imediatamente — todo o trabalho é assíncrono.
  ...
  Future<void> _runAsync() async {
    _running = true;
    try {
      // 1. Aplicar update pendente da sessão anterior (se existir)
      await _applyPendingUpdateIfExists();

      // 2. Verificar se já passou o intervalo mínimo desde o último check
      if (!await _shouldCheck()) {
        _running = false;
        return;
      }

      // 3. Verificar sample de pokémon contra a API
      final hasChanges = await _checkSample();

      // 4. Se detectou mudanças, baixar tudo e salvar como pending
      if (hasChanges) {
        await _downloadAndSavePending();
      }

      // Registrar horário do check
      final prefs = await SharedPreferences.getInstance();
      await prefs.setInt(_lastCheckKey,
  ...
  // ─── Aplicar update pendente ────────────────────────────────────

  /// Se há um arquivo pendente da sessão anterior, move para o lugar certo
  /// e recarrega os dados em memória antes do app usar qualquer tela.
  Future<void> _applyPendingUpdateIfExists() async {
    try {
      ...
      // Validar que o JSON é válido antes de aplicar
      final raw = await pendingFile.readAsString();
      final decoded = json.decode(raw) as Map<String, dynamic>;
      if (decoded.length < 100) {
        // Arquivo suspeito — ignorar
        await pendingFile.delete();
        await prefs.remove(_pendingKey);
        return;
      }

      // Substituir o arquivo ativo
      final activeFile = File('${dir.path}/pokedex_data_override.json');
      await pendingFile.rename(activeFile.path);
      await prefs.remove(_pendingKey);

      // Recarregar em memória — será usado na PRÓXIMA abertura do app
      // (não afeta a sessão atual pois _loaded já é true)
    } catch (_) {}
  }
  ...
  Future<bool> _checkSample() async {
    ...
    // Seleciona IDs distribuídos uniformemente (não aleatórios — reproduzível)
    final step = 1025 ~/ _sampleSize;
    final sampleIds = List.generate(
        _sampleSize, (i) => (i * step) + 1);

    int divergencias = 0;

    for (int i = 0; i < sampleIds.length; i += _batchSize) {
      final batch = sampleIds.skip(i).take(_batchSize).toList();
      final results = await Future.wait(
          batch.map((id) => _checkOne(svc, id)));
      divergencias += results.where((changed) => changed).length;

      // Se já encontrou 2+ divergências, não precisa checar mais
  ...
  Future<bool> _checkOne(PokedexDataService svc, int id) async {
    try {
      final r = await http.get(Uri.parse('$_base/pokemon/$id'))
          .timeout(const Duration(seconds: 8));
      if (r.statusCode != 200) return false;

      final remote = json.decode(r.body) as Map<String, dynamic>;
      final local  = svc.get(id);
      if (local == null) return true; // ID não existe localmente

      // Comparar campos chave
      final remoteTypes = (remote['types'] as List<dynamic>)
          .map((t) => t['type']['name'] as String).toList();
      final localTypes  = (local['types'] as List<dynamic>?)
          ?.cast<String>() ?? [];

      final remoteHeight = '${(remote['height'] as int) / 10} m';
      final localHeight  = local['height'] as String? ?? '';

      // Detecta mudança se tipos ou altura divergirem
      return remoteTypes.join(',') != localTypes.join(',') ||
             remoteHeight != localHeight;
    } catch (_) {
      return false; // timeout ou sem rede — não considera mudança
    }
  }

  // ─── Download completo e salvar como pending ────────────────────

  Future<void> _downloadAndSavePending() async {
    final allData = <int, Map<String, dynamic>>{};

    // Baixar /pokemon + /species para todos os 1025
    for (int i = 1; i <= 1025; i += _batchSize) {
      final batch = List.generate(
          _batchSize, (j) => i + j).where((id) => id <= 1025).toList();
      ...
      // Pausa entre batches — não stressar a rede
      await Future.delayed(const Duration(milliseconds: 400));
    }

    if (allData.length < 900) return; // download incompleto — descartar

    // Salvar como arquivo pendente
    final dir = await getApplicationDocumentsDirectory();
    final pending = File('${dir.path}/$_pendingFileName');
    await pending.writeAsString(
        json.encode(allData.map((k, v) => MapEntry(k.toString(), v))));
  ...
  Future<Map<String, dynamic>?> _fetchCompact(int id) async {
    try {
      ...
      for (final g in (s['genera'] as List<dynamic>? ?? [])) {
        if (g['language']['name'] == 'pt-BR') { category = g['genus']; break; }
      }
      if (category.isEmpty) {
        for (final g in (s['genera'] as List<dynamic>? ?? [])) {
          if (g['language']['name'] == 'en') { category = g['genus']; break; }
        }
      }

      String flavorEn = '';
      for (final e in (s['flavor_text_entries'] as List<dynamic>? ?? []).reversed.toList()) {
