...
const Map<String, List<String>> _formaKeyTypes = {
  ...
  // Mega type changes (only entries that differ from base)
  '6_MEGAX':    ['fire','dragon'],
  '80_MEGA':    ['water','psychic'],
  '127_MEGA':   ['bug','flying'],
  '130_MEGA':   ['water','dark'],
  '181_MEGA':   ['electric','dragon'],
  '306_MEGA':   ['steel'],
  '334_MEGA':   ['dragon','fairy'],
  '428_MEGA':   ['normal','fighting'],
  '531_MEGA':   ['normal','fairy'],
  '383_PRIMAL': ['fire','ground'],
  // Rotom forms
  '479_HEAT':  ['electric','fire'],
  '479_WASH':  ['electric','water'],
  '479_FROST': ['electric','ice'],
  '479_FAN':   ['electric','flying'],
  '479_MOW':   ['electric','grass'],
  // Castform weather
  '351_SUNNY': ['fire'],
  '351_RAINY': ['water'],
  '351_SNOWY': ['ice'],
  // Wormadam
  '413_SANDY': ['bug','ground'],
  '413_TRASH': ['bug','steel'],
  // Deoxys (all psychic)
  '386_ATTACK':   ['psychic'],
  '386_DEFENSE':  ['psychic'],
  '386_SPEED':    ['psychic'],
  // Shaymin-Sky
  '492_SKY': ['grass','flying'],
  // Aegislash-Blade
  '681_BLADE': ['steel','ghost'],
  // Therian forms
  '641_THERIAN': ['flying'],
  '642_THERIAN': ['electric','flying'],
  '645_THERIAN': ['ground','flying'],
  '905_THERIAN': ['fairy','flying'],
  // Necrozma
  '800_DUSK':  ['psychic','steel'],
  '800_DAWN':  ['psychic','ghost'],
  '800_ULTRA': ['psychic','dragon'],
  // Calyrex riders
  '898_ICE':    ['psychic','ice'],
  '898_SHADOW': ['psychic','ghost'],
  // Urshifu Rapid-Strike
  '892_RAPID': ['fighting','water'],
  // Zacian / Zamazenta crowned
  '888_CROWNED': ['fairy','steel'],
  '889_CROWNED': ['fighting','steel'],
  // Hoopa Unbound
  '720_UNBOUND': ['psychic','dark'],
  // Ogerpon masks
  '1017_WELLSPRING':  ['grass','water'],
  '1017_HEARTHFLAME': ['grass','fire'],
  '1017_CORNERSTONE': ['grass','rock'],
  // Wishiwashi School
  '746_SCHOOL': ['water'],
  // Zygarde
  '718_10':       ['dragon','ground'],
  '718_COMPLETE': ['dragon','ground'],
};
...
class _Entry {
  ...
  final int? formId;      // PokéAPI form ID — não-nulo só para formas da Nacional

  _Entry({required this.entryNumber, required this.speciesId, this.formaKey, this.formId});

  String get catchKey =>
      formId != null ? 'form_$formId' :
      formaKey != null ? '${speciesId}_$formaKey' : '$speciesId';

  bool get isNacionalForm => formId != null;
}

// ─── FORMAS ALTERNATIVAS ─────────────────────────────────────────
// Converte o slug de nome do PokéAPI (ex: "raichu-alola") no sufixo
// usado pelo sprite local (ex: "ALOLA"). Retorna null para formas sem sprite.

String? _extractFormSuffix(String formName) {
  const patterns = <String, String>{
    'paldea-combat-breed': 'PALDEA_COMBAT',
    'paldea-blaze-breed':  'PALDEA_BLAZE',
    'paldea-aqua-breed':   'PALDEA_AQUA',
    'rapid-strike-gmax':   'GMAX_RAPID',
    'single-strike-gmax':  'GMAX',
    'rapid-strike':        'RAPID',
  ...
  for (final entry in patterns.entries) {
    if (formName.endsWith('-${entry.key}')) return entry.value;
  }
  return null;
}
...
String _suffixToPt(String suffix) {
  switch (suffix) {
    ...
    case 'GALARIAN':     return 'de Galar';
    case 'HISUI':        return 'de Hisui';
    case 'PALDEA':       return 'de Paldea';
    case 'PALDEA_COMBAT':return 'de Paldea (Combate)';
    case 'PALDEA_BLAZE': return 'de Paldea (Chama)';
    case 'PALDEA_AQUA':  return 'de Paldea (Aqua)';
    case 'MEGA':         return 'Mega';
    case 'MEGAX':        return 'Mega X';
    case 'MEGAY':        return 'Mega Y';
    case 'MEGAZ':        return 'Mega Z';
    case 'GMAX':         return 'Gigantamax';
    case 'GMAX_RAPID':   return 'Gigantamax (Golpe Rápido)';
    case 'RAPID':        return '(Golpe Rápido)';
    case 'PRIMAL':       return 'Primal';
    case 'ORIGIN':       return '(Origem)';
    case 'SKY':          return '(Céu)';
    case 'THERIAN':      return '(Encarnação Divina)';
    case 'BLACK':        return 'Preto';
    case 'WHITE':        return 'Branco';
    case 'RESOLUTE':     return '(Resoluto)';
    case 'PIROUETTE':    return '(Pirueta)';
    case 'BLADE':        return '(Espada)';
    case 'UNBOUND':      return '(Desacorrentado)';
    case 'SUNNY':        return '(Sol)';
    case 'RAINY':        return '(Chuva)';
    case 'SNOWY':        return '(Neve)';
    case 'HEAT':         return '(Calor)';
    case 'WASH':         return '(Lavagem)';
    case 'FROST':        return '(Gelo)';
    case 'FAN':          return '(Ventilação)';
    case 'MOW':          return '(Corte)';
    case 'SCHOOL':       return '(Cardume)';
    case 'MIDNIGHT':     return '(Meia-Noite)';
    case 'DUSK':         return '(Crepúsculo)';
    case 'DAWN':         return '(Amanhecer)';
    case 'ULTRA':        return 'Ultra';
    case 'ORIGINAL':     return '(Original)';
    case 'CROWNED':      return '(Coroado)';
    case 'ETERNAMAX':    return 'Eternamax';
    case 'ICE':          return '(Cavaleiro do Gelo)';
    case 'SHADOW':       return '(Cavaleiro das Sombras)';
    case 'HERO':         return '(Herói)';
    case 'SANDY':        return '(Areia)';
    case 'TRASH':        return '(Lixo)';
    case 'COMPLETE':     return '(Completo)';
    case '10':           return '10%';
    case 'WELLSPRING':   return '(Máscara da Fonte)';
    case 'HEARTHFLAME':  return '(Máscara da Lareira)';
    case 'CORNERSTONE':  return '(Máscara da Pedra)';
    case 'ATTACK':       return '(Ataque)';
    case 'DEFENSE':      return '(Defesa)';
    case 'SPEED':        return '(Velocidade)';
    case 'BLOODMOON':    return '(Lua de Sangue)';
    default: return suffix;
  }
}

// formaKeys com sprites disponíveis em assets/sprites/artwork/
const Set<String> _knownFormSprites = {
  '3_GMAX','3_MEGA','6_GMAX','6_MEGAX','6_MEGAY','9_GMAX','9_MEGA',
  '12_GMAX','15_MEGA','18_MEGA','19_ALOLA','20_ALOLA','25_GMAX',
  '26_ALOLA','26_MEGAX','26_MEGAY','27_ALOLA','28_ALOLA','36_MEGA',
  '37_ALOLA','38_ALOLA','50_ALOLA','51_ALOLA','52_ALOLA','52_GALARIAN',
  '52_GMAX','53_ALOLA','58_HISUI','59_HISUI','65_MEGA','68_GMAX',
  '71_MEGA','74_ALOLA','75_ALOLA','76_ALOLA','77_GALARIAN','78_GALARIAN',
  '79_GALARIAN','80_GALARIAN','80_MEGA','83_GALARIAN','88_ALOLA','89_ALOLA',
  '94_GMAX','94_MEGA','99_GMAX','100_HISUI','101_HISUI','103_ALOLA',
  '105_ALOLA','110_GALARIAN','115_MEGA','121_MEGA','122_GALARIAN','127_MEGA',
  '128_PALDEA_AQUA','128_PALDEA_BLAZE','128_PALDEA_COMBAT','130_MEGA',
  '131_GMAX','142_MEGA','143_GMAX','144_GALARIAN','145_GALARIAN','146_GALARIAN',
  '149_MEGA','150_MEGAX','150_MEGAY','154_MEGA','157_HISUI','160_MEGA',
  '181_MEGA','194_PALDEA','195_PALDEA','199_GALARIAN','208_MEGA','211_HISUI',
  '212_MEGA','214_MEGA','215_HISUI','222_GALARIAN','227_MEGA','229_MEGA',
  '248_MEGA','254_MEGA','257_MEGA','260_MEGA','263_GALARIAN','264_GALARIAN',
  '282_MEGA','302_MEGA','303_MEGA','306_MEGA','308_MEGA','310_MEGA',
  '319_MEGA','323_MEGA','334_MEGA','351_RAINY','351_SNOWY','351_SUNNY',
  '354_MEGA','358_MEGA','359_MEGA','359_MEGAZ','362_MEGA','373_MEGA',
  '376_MEGA','380_MEGA','381_MEGA','382_PRIMAL','383_PRIMAL','384_MEGA',
  '386_ATTACK','386_DEFENSE','386_SPEED','398_MEGA','413_SANDY','413_TRASH',
  '428_MEGA','445_MEGA','445_MEGAZ','448_MEGA','448_MEGAZ','460_MEGA',
  '475_MEGA','478_MEGA','479_FAN','479_FROST','479_HEAT','479_MOW','479_WASH',
  '485_MEGA','487_ORIGIN','491_MEGA','492_SKY','500_MEGA','503_HISUI',
  '530_MEGA','531_MEGA','545_MEGA','549_HISUI','554_GALARIAN','555_GALARIAN',
  '560_MEGA','562_GALARIAN','569_GMAX','570_HISUI','571_HISUI','604_MEGA',
  '609_MEGA','618_GALARIAN','623_MEGA','628_HISUI','641_THERIAN','642_THERIAN',
  '645_THERIAN','646_BLACK','646_WHITE','647_RESOLUTE','648_PIROUETTE',
  '652_MEGA','655_MEGA','658_MEGA','668_MEGA','669_MEGA','678_MEGA',
  '681_BLADE','687_MEGA','689_MEGA','691_MEGA','701_MEGA','705_HISUI',
  '706_HISUI','706_MEGA','718_10','718_COMPLETE','718_MEGA','719_MEGA',
  '720_UNBOUND','724_HISUI','740_MEGA','745_DUSK','745_MIDNIGHT','746_SCHOOL',
  '768_MEGA','780_MEGA','796_MEGA','800_DAWN','800_DUSK','800_ULTRA',
  '801_MEGA','801_ORIGINAL','809_GMAX','812_GMAX','815_GMAX','818_GMAX',
  '823_GMAX','826_GMAX','834_GMAX','839_GMAX','841_GMAX','841_MEGA',
  '842_GMAX','844_GMAX','845_GMAX','849_GMAX','851_GMAX','858_GMAX',
  '861_GMAX','870_MEGA','879_GMAX','884_GMAX','887_GMAX','888_CROWNED',
  '889_CROWNED','890_ETERNAMAX','892_GMAX','892_GMAX_RAPID','892_RAPID',
  '898_ICE','898_SHADOW','905_THERIAN','964_HERO','971_MEGA','978_MEGA',
  '1017_CORNERSTONE','1017_HEARTHFLAME','1017_WELLSPRING',
};
...
class _PokedexScreenState extends State<PokedexScreen>
  ...
  void _onFormsSettingChanged() {
    if (!_isNacional || !mounted) return;
    setState(() {
      _entriesBySection = {};
      _visibleEntries   = [];
      _currentPage      = 0;
  ...
  Future<void> _initPokedex() async {
    ...
    try {
      ...
      Map<String, dynamic> _entryToMap(_Entry e) => <String, dynamic>{
        'speciesId': e.speciesId,
        'catchKey':  e.catchKey,
        if (e.formId != null) 'formId': e.formId,
      };

      // Carrega status de captura apenas da primeira página antes de renderizar
      final filtered = _allFilteredEntries();
      final firstPageEntries = filtered
          .take(_pageSize)
          .map(_entryToMap)
          .toList();
      final firstPageMap = await _storage.getCaughtMapByEntries(_effectivePokedexId, firstPageEntries);

      if (!mounted) return;
      setState(() {
        _caughtMap.addAll(firstPageMap);
        _loadingIds = false;
      });

      await _loadPage(0);

      // Carrega o restante do caught map em background após a UI estar visível
      if (filtered.length > _pageSize) {
        final remainingEntries = filtered
            .skip(_pageSize)
            .map(_entryToMap)
            .toList();
        _storage.getCaughtMapByEntries(_effectivePokedexId, remainingEntries).then((rest) {
          if (!mounted) return;
          setState(() => _caughtMap.addAll(rest));
        });
      }
    } catch (e) {
      if (!mounted) return;
    ...
    if (_searchQuery.isNotEmpty) {
      ...
      entries = entries.where((e) {
        ...
        for (final t in svc.getTypes(e.speciesId)) {
          final namePt = typeName(t).toLowerCase();
  ...
  Map<String, dynamic> _localPokemonData(int id, {String? formaKey}) {
    ...
    final name = formaKey != null && _formaKeyNames.containsKey(formaKey)
        ...
        : formaKey != null
            ? _buildFormDisplayName(id, formaKey)
  ...
  String _buildFormDisplayName(int speciesId, String formaKey) {
    final baseName = _pokemonNameFromId(speciesId);
    final idx = formaKey.indexOf('_');
    if (idx < 0) return baseName;
    return '$baseName ${_suffixToPt(formaKey.substring(idx + 1))}';
  }
  ...
  Future<void> _loadPage(int page) async {
    ...
    // Pré-decodifica imagens após o primeiro frame (não bloqueia a UI)
  ...
  Future<void> _toggleCatch(_Entry entry) async {
    final catchKey = entry.catchKey;
    final current = _caughtMap[catchKey] ?? false;
    final newVal = !current;
    HapticFeedback.mediumImpact();
    setState(() => _caughtMap[catchKey] = newVal);
    if (entry.isNacionalForm) {
      await _storage.setFormCaught(entry.formId!, newVal);
    } else {
      await _storage.setCaught(_effectivePokedexId, entry.speciesId, newVal);
    }
  }
  ...
  Future<void> _openDetailAt(List<_Entry> filtered, int idx) async {
    ...
    final onToggle = () async {
      isCaught = !isCaught;
      if (entry.isNacionalForm) {
        await _storage.setFormCaught(entry.formId!, isCaught);
      } else {
    ...
    if (mounted) {
      final updated = entry.isNacionalForm
          ? await _storage.isFormCaught(entry.formId!)
          : await _storage.isCaught(_effectivePokedexId, entry.speciesId);
          ...
          itemBuilder: (context, index) {
            ...
            return ValueListenableBuilder<String>(
              ...
              builder: (_, sprite, __) => _PokemonCard(
                ...
                onLongPress: () => _toggleCatch(entry),
                onToggle: () => _toggleCatch(entry),
...
class _PokemonCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color1 = typeColor(types[0]);
    final color2 = types.length > 1 ? typeColor(types[1]) : color1;
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Badges com ícone — fonte 7 para caber nomes longos em 52px
                  Row(
                    mainAxisAlignment: MainAxisAlignment.center,
